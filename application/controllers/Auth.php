...
class Auth extends CI_Controller
{
        ...
        private function _sendEmail($token, $type)
        {
                ...
                $config['smtp_user'] = 'fitrimaulidi33@gmail.com'; //email yang dirubah keamanannya
                $config['smtp_pass'] = 'wbqzanzfcahltmvv'; //isikan password dr kelola akun gmail
                $config['smtp_port'] = 465;
                $config['mailtype'] = 'html';
                $config['charset'] = 'utf-8';
                // $config = array(
                //        'protocol'=> 'smtp',
                //        'smtp_host' => 'smtp.mailtrap.indo',
                //        'smtp_port' => 2525
                //        'smtp_user' => '51739a034cb41;,                                  
                ...
                $this->email->from('fitrimaulidi33@gmail.com', 'Web Programing SMKN8Jember');
                $this->email->to($this->input->post('email'));

                if ($type == 'verify') {
                        $this->email->subject('Account Verivication');
                        $this->email->message('Clik this link to verify you accoun :
                                        <a href="' . base_url() . 'auth/verify?email=' . $this->input->post('emai') .
                                '&token=' . urlencode($token) . '">Activated</a>');
                } else if ($type == 'forgot') {
                        $this->email->subject('Reset Password');
                        $this->email->message('Click this link ton seret your password :
                        ...
                        if ($user_token) {
                                if (time() - $user_token['date_created'] < (60 * 60 * 24)) {
                                        ...
                                        $this->session->set_flashdata('message', '<div class="alert alert-success"
                                                 role="alert">Account activation failed!Wrong expired!.</div>');
                                        redirect('auth');
                                } else {
